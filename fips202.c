 ...
 *
 * Description: Initilizes Keccak context for use as SHAKE128 XOF
 *
 * Arguments:   - shake128incctx *ctx: pointer to (uninitialized) Keccak context
 **************************************************/
 ...
 *
 * Arguments:   - shake128incctx *ctx: pointer to (initialized) Keccak context
 *              - const uint8_t *in: pointer to input to be absorbed into s
 *              - size_t inlen: length of input in bytes
 **************************************************/
void shake128_inc_absorb(shake128incctx *ctx, const uint8_t *in, size_t inlen) {
  ctx->pos = keccak_absorb(ctx->s, ctx->pos, SHAKE128_RATE, in, inlen);
}
 ...
 *
 * Arguments:   - shake128incctx *ctx: pointer to Keccak context
 **************************************************/
void shake128_inc_finalize(shake128incctx *ctx) {
  keccak_finalize(ctx->s, ctx->pos, SHAKE128_RATE, 0x1F);
  ctx->pos = SHAKE128_RATE;
}
 ...
 *output)
 *              - shake128incctx *s: pointer to Keccak context
 **************************************************/
void shake128_inc_squeeze(uint8_t *out, size_t outlen, shake128incctx *ctx) {
  ctx->pos = keccak_squeeze(out, outlen, ctx->s, ctx->pos, SHAKE128_RATE);
}

/*************************************************
 * Name:        shake128_inc_absorb_once
 *
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
void shake128_inc_absorb_once(shake128incctx *ctx, const uint8_t *in,
                              size_t inlen) {
  keccak_absorb_once(ctx->s, SHAKE128_RATE, in, inlen, 0x1F);
  ctx->pos = SHAKE128_RATE;
}

/*************************************************
 * Name:        shake128_inc_squeezeblocks
 *
 * Description: Squeeze step of SHAKE128 XOF. Squeezes full blocks of
 *              SHAKE128_RATE bytes each. Can be called multiple times
 *              to keep squeezing. Assumes new block has not yet been
 *              started (ctx->pos = SHAKE128_RATE).
 *
 * Arguments:   - uint8_t *out: pointer to output blocks
 *              - size_t nblocks: number of blocks to be squeezed (written to
 *output)
 *              - shake128incctx *s: pointer to Keccak context
 **************************************************/
void shake128_inc_squeezeblocks(uint8_t *out, size_t nblocks,
                                shake128incctx *ctx) {
  keccak_squeezeblocks(out, nblocks, ctx->s, SHAKE128_RATE);
}

/*************************************************
 * Name:        shake256_inc_init
 *
 * Description: Initilizes Keccak context for use as SHAKE256 XOF
 *
 * Arguments:   - shake256incctx *ctx: pointer to (uninitialized) Keccak context
 **************************************************/
 ...
 *
 * Arguments:   - shake256incctx *ctx: pointer to (initialized) Keccak context
 *              - const uint8_t *in: pointer to input to be absorbed into s
 *              - size_t inlen: length of input in bytes
 **************************************************/
void shake256_inc_absorb(shake256incctx *ctx, const uint8_t *in, size_t inlen) {
  ctx->pos = keccak_absorb(ctx->s, ctx->pos, SHAKE256_RATE, in, inlen);
}
 ...
 *
 * Arguments:   - shake256incctx *ctx: pointer to Keccak context
 **************************************************/
void shake256_inc_finalize(shake256incctx *ctx) {
  keccak_finalize(ctx->s, ctx->pos, SHAKE256_RATE, 0x1F);
  ctx->pos = SHAKE256_RATE;
}
 ...
 *              - size_t outlen : number of bytes to be squeezed (written to
 *output)
 *              - shake256incctx *s: pointer to Keccak context
 **************************************************/
void shake256_inc_squeeze(uint8_t *out, size_t outlen, shake256incctx *ctx) {
  ctx->pos = keccak_squeeze(out, outlen, ctx->s, ctx->pos, SHAKE256_RATE);
}

/*************************************************
 * Name:        shake256_inc_absorb_once
 *
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
void shake256_inc_absorb_once(shake256incctx *ctx, const uint8_t *in,
                              size_t inlen) {
  keccak_absorb_once(ctx->s, SHAKE256_RATE, in, inlen, 0x1F);
  ctx->pos = SHAKE256_RATE;
}

/*************************************************
 * Name:        shake256_inc_squeezeblocks
 *
 * Description: Squeeze step of SHAKE256 XOF. Squeezes full blocks of
 *              SHAKE256_RATE bytes each. Can be called multiple times
 *              to keep squeezing. Assumes next block has not yet been
 *              started (ctx->pos = SHAKE256_RATE).
 *
 * Arguments:   - uint8_t *out: pointer to output blocks
 *              - size_t nblocks: number of blocks to be squeezed (written to
 *output)
 *              - shake256incctx *s: pointer to Keccak context
 **************************************************/
void shake256_inc_squeezeblocks(uint8_t *out, size_t nblocks,
                                shake256incctx *ctx) {
  keccak_squeezeblocks(out, nblocks, ctx->s, SHAKE256_RATE);
}
...
void shake128(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen) {
  ...
  shake128_inc_absorb_once(&ctx, in, inlen);
  nblocks = outlen / SHAKE128_RATE;
  shake128_inc_squeezeblocks(out, nblocks, &ctx);
...
void shake256(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen) {
  ...
  shake256_inc_absorb_once(&ctx, in, inlen);
  nblocks = outlen / SHAKE256_RATE;
  shake256_inc_squeezeblocks(out, nblocks, &ctx);
