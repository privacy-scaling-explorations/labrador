...
static inline void vaesni_encrypt8(uint8_t out[512], __m128i *n,
                                   const __m128i *rkeys, int rounds) {
  int i;
  ...
  _mm_store_si128(n, _mm_add_epi64(_mm512_castsi512_si128(f5),
                                   _mm512_castsi512_si128(off3)));
...
void aes128ctr_init(aes128ctr_ctx *state, const uint8_t key[16],
                    uint64_t nonce) {
...
void aes128ctr_squeezeblocks(uint8_t *out, size_t nblocks,
                             aes128ctr_ctx *state) {
  size_t i;
  for (i = 0; i < nblocks; i++) {
