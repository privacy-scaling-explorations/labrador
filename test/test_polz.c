...
static void polz_kroneckermul_extension(polz *r, const polz *a, const polz *b,
                                        size_t deg) {
  ssize_t i, j, k;
  mpz_t az, bz, qz, tz, uz;
...
int main(void) {
  ...
  __attribute__((aligned(16))) uint8_t buf[N * QBYTES];
  uint64_t nonce = 0;
  polz a, b, c, d;
  polx x;
  poly y[LOGQ];
  ...
  printf("norm uniform: %.2g (rel dist to expected: %.2f)\n", l,
         fabs(1 - l / ldexp(sqrt(N / 12.0), LOGQ)));
