...
void polxvec_fromint64vec(polx *r, size_t len, size_t deg,
...
void polxvec_ternary(polx *r, size_t len, const uint8_t seed[16],
                     uint64_t nonce) {
  poly t[32];
...
void polxvec_quarternary(polx *r, size_t len, const uint8_t seed[16],
                         uint64_t nonce) {
  poly t[32];
...
void polxvec_challenge(polx *r, size_t len, const uint8_t seed[16],
                       uint64_t nonce) {
  poly t[10];
...
void polxvec_almostuniform(polx *r, size_t len, const uint8_t seed[16],
                           uint64_t nonce) {
  polz t[32];
...
void polxvec_polx_mul(polx *r, const polx *a, const polx *b, size_t len) {
  ...
  for (i = 0; i < K; i++)
    polyvec_poly_pointwise(&r->vec[i], &a->vec[i], &b->vec[i], len, K,
                           &primes[i]);
}
...
void polxvec_mul_add(polx *r, const polx *a, const polx *b, size_t len) {
  ...
  for (i = 0; i < K; i++)
    polyvec_pointwise_add(&r->vec[i], &a->vec[i], &b->vec[i], len, K,
                          &primes[i]);
...
void polxvec_polx_mul_add(polx *r, const polx *a, const polx *b, size_t len) {
  ...
  for (i = 0; i < K; i++)
    polyvec_poly_pointwise_add(&r->vec[i], &a->vec[i], &b->vec[i], len, K,
                               &primes[i]);
...
void polxvec_sprod(polx *r, const polx *a, const polx *b, size_t len) {
  ...
  for (i = 0; i < K; i++)
    polyvec_sprod_pointwise(&r->vec[i], &a->vec[i], &b->vec[i], len, K,
                            &primes[i]);
}
...
void polxvec_sprod_add(polx *r, const polx *a, const polx *b, size_t len) {
  ...
  for (i = 0; i < K; i++)
    polyvec_sprod_pointwise_add(&r->vec[i], &a->vec[i], &b->vec[i], len, K,
                                &primes[i]);
}
...
void polxvec_scale_frompolyvec(polx *r, const poly *a, size_t len, int64_t s) {
  ...
  s = cmodq(s);
  for (i = 0; i < K; i++)
    polyvec_scale_widening(&r->vec[i], a, len, K, s * primes[i].s % primes[i].p,
                           &primes[i]);
...
void polxvec_scale_add(polx *r, const polx *a, size_t len, int64_t s) {
  ...
  s <<= 16;
  for (i = 0; i < K; i++)
    polyvec_scale_add(&r->vec[i], &a->vec[i], len, K, s % primes[i].p,
                      &primes[i]);

  polxvec_reduce(r, len);
}

size_t polxvec_mul_extension(polx *c, const polx *a, const polx *b, size_t len,
                             size_t deg, size_t mult) {
  size_t i, j, k = 0;

  for (i = 0; i < mult; i++)
    for (j = 0; j < K; j++)
      k = polyvec_pointwise_extension(&c[i].vec[j], &a->vec[j], &b[i].vec[j],
                                      len, K * mult, deg, &primes[j]);

  return k;
}

size_t polxvec_collaps_add_extension(polx *c, const polx *a, const polx *b,
                                     size_t len, size_t deg, size_t mult) {
  size_t i, j, k = 0;

  for (i = 0; i < mult; i++)
    for (j = 0; j < K; j++)
      k = polyvec_collaps_add_extension(&c[i].vec[j], &a[i].vec[j], &b->vec[j],
                                        len, K * mult, deg, &primes[j]);
...
void polxvec_reconstruct(polx *r, const poly *a, size_t len, size_t t,
                         size_t d) {
  size_t i;
  const size_t stride = len;
  polz b[16];
