...
static int init_polcomctx(polcomctx *ctx, size_t len) {
  ...
  for (cpp->f = 2; cpp->f <= 8; cpp->f++) {
    ...
    for (cpp->kappa = 1; cpp->kappa <= 32; cpp->kappa++) {
      ...
      // cpp->bu = round(0.5*cpp->b + 0.25*log2(ctx->n) +
      // 0.25*log2(TAU1+4*TAU2)));
      cpp->fu = round((double)LOGQ / cpp->bu);

      ctx->normsq =
          (exp2(2 * cpp->bu) / 12 + varz / exp2(2 * cpp->bu)) * ctx->m * cpp->f;
      ctx->normsq += (exp2(2 * cpp->bu) * (cpp->fu - 1) +
                      exp2(2 * (LOGQ - (cpp->fu - 1) * cpp->bu))) /
                     12 * (cpp->kappa + 1) * ctx->n;
      ctx->normsq *= N;

      if (sis_secure(cpp->kappa,
                     6 * T * SLACK * exp2(cpp->bu) * sqrt(ctx->normsq)))
        break;
    }
  ...
  if (cpp->kappa > 32) {
    fprintf(
        stderr,
        "ERROR in init_polcomctx(): Cannot make inner commitments secure!\n");
    return 1;
  }
  if (cpp->kappa1 > 32) {
    fprintf(
        stderr,
        "ERROR in init_polcomctx(): Cannot make outer commitments secure!\n");
    return 2;
  }
...
static void init_polcomprf(polcomprf *pi, const polcomctx *ctx, uint32_t x,
                           uint32_t y) {
  pi->len = ctx->len;
  pi->m = ctx->m;
  pi->n = ctx->n;
...
int polcom_commit(polcomctx *ctx, const polz *s, size_t len) {
  ...
  if (ret)
    return ret; // commitments not secure (1/2)
  ...
  size_t i, j;
  __attribute__((aligned(16))) uint8_t hashbuf[cpp->kappa1 * N * QBYTES];
  ...
  for (i = 0; i < n; i++) {
    /* inner commitments */
    polzvec_decompose_topolxvec(sx[i], &s[i * m], MIN(m, len - i * m), m,
                                cpp->f, cpp->b);
    polxvec_mul_extension(t, comkey, sx[i], m * cpp->f, cpp->kappa, 1);
    polxvec_decompose(&ctx->t[i * cpp->kappa * cpp->fu], t, cpp->kappa, cpp->fu,
                      cpp->bu);

    /* outer commitment */
    polxvec_frompolyvec(t, &ctx->t[i * cpp->kappa * cpp->fu],
                        cpp->kappa * cpp->fu);
    if (i == 0)
      j += polxvec_mul_extension(u, &comkey[j], t, cpp->kappa * cpp->fu,
                                 cpp->kappa1, 1);
    else {
      j += polxvec_mul_extension(t, &comkey[j], t, cpp->kappa * cpp->fu,
                                 cpp->kappa1, 1);
...
void polcom_eval(witness *wt, polcomprf *pi, const polcomctx *ctx, int64_t x,
  ...
  __attribute__((
      aligned(16))) uint8_t hashbuf[16 + (2 + cpp->kappa1 * N) * QBYTES];
  ...
  for (i = 0; i < n; i++) {
    ...
    */
    if (!ctx->sx)
      polzvec_decompose_topolxvec(sx[i], &ctx->s[i * m],
                                  MIN(m, ctx->len - i * m), m, cpp->f, cpp->b);
    for (k = 0; k < cpp->f; k++) {
  ...
  /* commit to w */
  polx *tmp = _aligned_alloc(
      64, MAX(MAX(wt->n[3], cpp->kappa1), n + m * cpp->f) * sizeof(polx));
...
int polcom_reduce(prncplstmnt *st, const polcomprf *pi) {
  ...
  size_t stn[4], len;
  __attribute__((aligned(16))) uint8_t hashbuf[24 + cpp->kappa1 * N * QBYTES];
  polx *buf;
  ...
  for (i = 1; i < cpp->fu; i++)
    polxvec_scale(&cnst->phi[2][i * n], &cnst->phi[2][0], n,
                  (int64_t)1 << i * cpp->bu);
  ...
  polxvec_scale(cnst->phi[1], comkey, extlen(st->n[1], cpp->kappa),
                (int64_t)1 << cpp->bu);
  ...
  for (i = 0; i < n; i++)
    for (j = 0; j < cpp->fu; j++)
      polxvec_copy(&cnst->phi[2][(i * cpp->fu + j) * cpp->kappa], &c[n * j + i],
                   1);
