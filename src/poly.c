...
void polyvec_fromint64vec(poly *r, size_t len, size_t deg,
                          const int64_t a[len * deg * N]) {
  size_t i, j, k;
...
static inline size_t uniform_ref(int16_t *r, size_t len, const uint8_t *buf,
                                 size_t buflen, int16_t p) {
  size_t i, j;
  int s;
  int16_t t, u;
...
static inline size_t uniform(int16_t *r, size_t len, const uint8_t *buf,
                             size_t buflen, int16_t p) {
  size_t i, j;
  const int bits = (p & 0x2000) ? 14 : 13;
  __m512i f, g;
  ...
  if (bits == 13) {
    permbidx = _mm512_set_epi8(
        51, 50, 49, 48, 48, 47, 46, 45, 44, 43, 43, 42, 41, 40, 40, 39, 38, 37,
        36, 35, 35, 34, 33, 32, 31, 30, 30, 29, 28, 27, 27, 26, 25, 24, 23, 22,
        22, 21, 20, 19, 18, 17, 17, 16, 15, 14, 14, 13, 12, 11, 10, 9, 9, 8, 7,
        6, 5, 4, 4, 3, 2, 1, 1, 0);
    srlvwidx = _mm512_broadcast_i64x2(_mm_set_epi16(3, 6, 1, 4, 7, 2, 5, 0));
    sllvwidx =
        _mm512_broadcast_i64x2(_mm_set_epi16(13, 10, 13, 12, 9, 13, 11, 13));
  } else {
    permbidx = _mm512_set_epi8(
        55, 54, 53, 52, 51, 50, 50, 49, 48, 47, 46, 45, 44, 43, 43, 42, 41, 40,
        39, 38, 37, 36, 36, 35, 34, 33, 32, 31, 30, 29, 29, 28, 27, 26, 25, 24,
        23, 22, 22, 21, 20, 19, 18, 17, 16, 15, 15, 14, 13, 12, 11, 10, 9, 8, 8,
        7, 6, 5, 4, 3, 2, 1, 1, 0);
    srlvwidx = _mm512_broadcast_i64x2(_mm_set_epi16(2, 4, 6, 0, 2, 4, 6, 0));
    sllvwidx =
        _mm512_broadcast_i64x2(_mm_set_epi16(14, 12, 10, 14, 14, 12, 10, 14));
  }
...
void polyvec_uniform(poly *r, size_t len, const pdata *prime,
  ...
  const int bits = (p & 0x2000) ? 14 : 13;
  size_t nblocks =
      ((32 * N * bits / 8 << bits) / p + AES128CTR_BLOCKBYTES - 1) /
      AES128CTR_BLOCKBYTES;
  __attribute__((aligned(64))) uint8_t buf[nblocks * AES128CTR_BLOCKBYTES];
  ...
  while (len) {
    nblocks = ((len * bits / 8 << bits) / p + AES128CTR_BLOCKBYTES - 1) /
              AES128CTR_BLOCKBYTES;
    aes128ctr_squeezeblocks(buf, nblocks, &aesctx);
...
/* Samples ternary polynomial with probabilities 5/16,6/16,5/16 for -1,0,1,
 * respectively (cbd(2) mod 3) */
void polyvec_ternary(poly *r, size_t len, const uint8_t seed[16],
                     uint64_t nonce) {
  size_t nblocks;
  uint8_t *buf;
  aes128ctr_ctx aesctx;
...
void polyvec_quarternary(poly *r, size_t len, const uint8_t seed[16],
                         uint64_t nonce) {
  size_t nblocks;
  uint8_t *buf;
  aes128ctr_ctx aesctx;
...
static inline size_t challenge(poly *c, size_t len, const uint8_t *buf,
                               size_t buflen) {
  size_t i, j;
  int k, b;
  uint64_t signs;
...
void polyvec_challenge(poly *c, size_t len, const uint8_t seed[16],
                       uint64_t nonce) {
  size_t k;
  __attribute__((aligned(64))) uint8_t buf[17 * SHAKE128_RATE];
...
int64_t polyvec_sprodz(const poly *a, const poly *b, size_t len) {
  ...
  t = _mm256_add_epi64(_mm512_castsi512_si256(acc),
...
void polyvec_quot_add(poly *r, const poly *a, size_t len, size_t stride,
                      const pdata *prime) {
  size_t i;
...
void polyvec_ntt(poly *r, const poly *a, size_t len, size_t stride,
                 const pdata *prime) {
  size_t i;
...
void polyvec_invntt(poly *r, const poly *a, size_t len, size_t stride,
                    const pdata *prime) {
  size_t i;
...
void polyvec_pointwise(poly *r, const poly *a, const poly *b, size_t len,
...
void polyvec_poly_pointwise(poly *r, const poly *a, const poly *b, size_t len,
                            size_t stride, const pdata *prime) {
  size_t i, j;
  __m512i f, g;
...
void poly_pointwise_add(poly *r, const poly *a, const poly *b,
                        const pdata *prime) {
  poly t;
...
void polyvec_pointwise_add(poly *r, const poly *a, const poly *b, size_t len,
                           size_t stride, const pdata *prime) {
  size_t i;
...
void polyvec_poly_pointwise_add(poly *r, const poly *a, const poly *b,
                                size_t len, size_t stride, const pdata *prime) {
  size_t i;
...
void polyvec_sprod_pointwise(poly *r, const poly *a, const poly *b, size_t len,
                             size_t stride, const pdata *prime) {
  size_t i;
...
static size_t bitrev(size_t k, size_t n) {
  const size_t t[32] = {0,  16, 8,  24, 4,  20, 12, 28, 2,  18, 10,
...
size_t polyvec_pointwise_extension(poly *c, const poly *a, const poly *b,
                                   size_t len, size_t stride, size_t deg,
                                   const pdata *prime) {
  size_t i, j, k;
  size_t deg2;
  const int extrared = (prime->p & 0x2000) ? 1 : 0;
  ...
  while (len) {
    for (i = 0; i < MIN(deg, len); i++) { // columns
      polyvec_poly_pointwise(&tmp[stride * 0], &b[stride * i],
                             &a[stride * (deg2 - i)], i, stride, prime);
      polyvec_poly_pointwise(&tmp[stride * i], &b[stride * i], &a[stride * 0],
                             deg - i, stride, prime);
      for (j = 0; j < i; j++)
    ...
    for (i = deg; i < MIN(deg2, len); i++) {
      polyvec_poly_pointwise(&tmp[stride * 0], &b[stride * i],
                             &a[stride * (deg2 - i)], deg, stride, prime);
      for (j = 0; j < deg; j++)
        poly_add(&tu[j], &tu[j], &tmp[stride * j]);
      if (1 || extrared || i % 2) // FIXME
...
size_t polyvec_collaps_add_extension(poly *c, const poly *a, const poly *b,
                                     size_t len, size_t stride, size_t deg,
                                     const pdata *prime) {
  size_t i, k;
  size_t deg2;
  ...
  while (len) {
    for (i = 0; i < MIN(deg, len); i++) {
      polyvec_sprod_pointwise_add(&c[stride * i], &tu[stride * 0],
                                  &b[stride * (deg2 - i)], i, stride, prime);
      polyvec_sprod_pointwise_add(&c[stride * i], &tl[stride * i],
                                  &b[stride * 0], deg - i, stride, prime);
    }
    for (i = deg; i < MIN(deg2, len); i++)
      polyvec_sprod_pointwise_add(&c[stride * i], &tu[stride * 0],
                                  &b[stride * (deg2 - i)], deg, stride, prime);
...
void polyvec_scale(poly *r, const poly *a, size_t len, size_t stride, int16_t s,
                   const pdata *prime) {
  size_t i;
...
void polyvec_scale_widening(poly *r, const poly *a, size_t len, size_t stride,
                            int16_t s, const pdata *prime) {
  size_t i;
...
void polyvec_scale_add(poly *r, const poly *a, size_t len, size_t stride,
                       int16_t s, const pdata *prime) {
  size_t i;
...
void poly_sigmam1(poly *r, const poly *a) {
  size_t i;
  __m512i f, g;
  const __m512i permwidx = _mm512_set_epi16(
...
void poly_sigma5(poly *r, const poly *a) { poly_sigma(r, a, 5); }
...
void poly_sigma5inv(poly *r, const poly *a) { poly_sigma(r, a, 3277); }
...
void polyvec_flip_ntt(poly *r, const poly *a, size_t len, size_t stride,
                      const pdata *prime) {
  size_t i;
